...
| P02 | Persistence and migrations | P0 | P01 | DONE | migrated SQLite + integrity/idempotency suite | Migration/model parity, FK/unique constraints, source-typed evidence, repeated import idempotency and transactional rollback verified |
| P03 | WPF shell/settings/localization | P0 | P02 | DONE | EN/AR resource parity + SQLite settings + Windows WPF smoke | DB-backed settings/audit, exact English/Arabic resource-key parity, runtime en-US/ar-KW culture, LTR/RTL switching, dark/light persistence and rendered shell smoke verified |
| P04 | Sync, resilience, audit | P0 | P02 | DONE | SQLite orchestration recovery + existing HTTP resilience suite | Ambiguous report creation is fail-closed, remote IDs survive retry/cancellation, explicit reconciliation gates fresh requests, terminal remote states close attempts, and audit persists safe codes only |
| P05 | Secure auth/capabilities | P0 | P04 | DONE | LWA refresh/capability + DPAPI-at-rest + connector HTTP suite | Separate Seller/Ads client IDs, DPAPI CurrentUser secret protection, one-time refresh after explicit 401, official read-only Seller/Ads capability probes, and credential-free connection audit; live owner credentials remain an external acceptance gate |
...
Convergence started from `main` commit `a1d0d67cd38f70daa1431ca21038cc98d2579c56`, whose Windows CI had 0 application warnings, 0 build errors and 20 passing tests. P02 audited the existing SQLite migration, EF model, importer, DI/configuration composition and persistence tests, then hardened source/fact-type invariants, migration/model parity, foreign/unique constraints, revision rollback and repeated-import idempotency. P03 audited the existing WPF shell and SQLite-backed settings instead of treating the existing surface as complete. The audit found incomplete Arabic resources, hard-coded English shell text, and language switching that changed resources/flow direction without setting WPF culture inheritance. P03 now enforces exact English/Arabic resource-key parity, binds `Window.Language` and `FlowDirection`, uses `en-US`/`ar-KW` runtime culture, localizes shell/status text, persists canonical theme/language preferences through `AppSetting`, and records setting-change audit events without values or credentials. P04 audited the existing `SyncCoordinator` and HTTP retry boundary and found two restart-safety defects: an ambiguous non-idempotent report POST could be followed by an automatic replacement request, and a failed/cancelled attempt with a persisted remote report ID could fall out of the resumable set. P04 now persists creation intent before POST, blocks automatic repost on uncertain outcomes, supports explicit remote-ID recovery or confirmed-absent reconciliation, resumes known remote IDs across failure/cancellation, recognizes terminal remote report states, increments failed artifact retry evidence, and records bounded safe audit/error codes without exception text or remote URLs. P04 Windows CI on commit `7bf65416bc30f121792da8d346e933d75fb643e3` passed the repository restore/Release build/test/WPF smoke gate. P05 audited DPAPI/LWA and connector configuration and found three security/capability gaps: Seller and Ads shared one client ID despite separate authorizations, an expired cached access token was not refreshed once after an explicit Amazon 401, and connection state was inferred from report work rather than verified through read-only entitlement checks. P05 separates Seller/Ads client IDs with legacy-setting fallback only for upgrade compatibility, keeps client secrets and refresh tokens in DPAPI CurrentUser storage, preserves HTTP status on auth failures, refreshes exactly once after an explicit 401 but never after 403 or ambiguous network/5xx report creation, probes Seller Reports through filtered `getReports`, probes Ads reporting access through the authorized profiles endpoint without a profile scope header, verifies Ads profile marketplace/currency, and audits only fixed connection state/codes. Production still performs no Amazon bid, budget, listing or campaign mutation, and browser automation remains fallback-only. Live owner credentials and entitlement verification remain external P19 acceptance evidence. Next dependency-valid P0 items are P06 Seller data, P07 Ads data and P17 Backup/recovery; P06 is next by dependency order and unlocks downstream Seller-dependent work.
